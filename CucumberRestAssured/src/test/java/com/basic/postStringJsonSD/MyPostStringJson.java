...
//		resp.then().log().status();
	...
	@Then("^validate the date format$")
	public void validate_the_date_format() throws Throwable {
	    
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		Date date = new Date(0);
		String todaysDate = dateFormat.format(date);
		
		String createdAtValue = resp.getBody().jsonPath().getString("createdAtValue");
		Assert.assertTrue(createdAtValue.contains(createdAtValue));
	}
